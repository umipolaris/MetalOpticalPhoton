...
s:Sc/U1v5_T/OutputFile = "U1v5_unpol_gpu_th{th}_T_sf1"
...
s:Sc/U1v5_R/OutputFile = "U1v5_unpol_gpu_th{th}_R_sf1"
...
TEMPLATE_CPU = TEMPLATE_GPU.replace(
    ...
    'U1v5_unpol_gpu_th', 'U1v5_unpol_cpu_th'
).replace(
...
def build_one(mode: str, th: int) -> Path:
    ...
    path = BUILT / f"U1v5_unpol_{mode}_th{th}.txt"
    if THREADS:
...
def main():
    print(f"Generating + running {len(ANGLES)*len(MODES)} U1v5_unpol jobs (modes={MODES}) ...")
